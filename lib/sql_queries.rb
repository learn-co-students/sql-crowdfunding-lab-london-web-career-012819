...
"SELECT projects.title, (SUM(pledges.amount) - (projects.funding_goal)) AS amount_over FROM projects JOIN pledges ON pledges.project_id = projects.id GROUP BY projects.title HAVING amount_over >= 0"
end
